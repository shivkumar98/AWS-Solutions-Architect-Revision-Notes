...
* D. 99.5 percent
   ...
   * **D**❌❌❌❌
   * **CORRECT ANSWER: B**
   * The Infrequent Access tier has 99.9% availability (https://aws.amazon.com/s3/storage-classes/#topic-4)
...
* D. Operations subsequent to the creation of a new object
   ...
   * **C**❌❌❌❌
   * **CORRECT ANSWER: D**
   * You do not need to worry about consistency concerns when creating new objects.
   </details>
...
* E. AWS Key Management Service
   ...
   * **A,B**❌❌❌❌
   * **CORRECT ANSWER:B,C**
   * S3 bucket and IAM policies provide secured access to buckets.
...
* E. AWS Key Management Service
   ...
   * **B,D**❌❌❌❌
   * **CORRECT ANSWER:A,D**
   * AWS Certificate Manager can be apply SSL/TLS encryption to the site.
   * Route 53 can be used to apply a DNS name (improving user experience).
   * EC2 and RDS do not apply to static website, and you would not use AWS KMS to encrypt anything for a website which has assets which need to be available to users.
...
<details>
   ...
   * **C**✅✅✅✅
   * Direct Connect is fast, but very expensive and can take up to 90 days
   * Server Migration Service and Storage Gateway are not designed for this scale of data.
   </details>
...
* D. Amazon Elastic Block Store
   ...
   * **A**✅✅✅✅
   * EBS volumes can't be accessed by more than 1 instance
   * Amazon FSx for Lustre and EFS are designed for Linux file systems
